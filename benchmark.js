const { cleanLinesArray, cleanLine } = require("./src/cleaner");
...
// Warmup
cleanLinesArray(lines.slice(0, 2000), { dedupe: false });

const start = Date.now();
const res = cleanLinesArray(lines, { dedupe: false });
const duration = Date.now() - start;

console.log(`Cleaned ${lines.length} lines in ${duration}ms (${Math.round(lines.length / (duration / 1000))} lines/sec)`);
