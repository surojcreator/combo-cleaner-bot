...
// Matches a plain username login: at least 3 chars, letters/digits/._-,
// starting with a letter OR starting with digits followed by at least one letter.
const USERNAME_RE =
    /^(?:[A-Za-z][A-Za-z0-9._-]{2,}|[0-9]+[A-Za-z][A-Za-z0-9._-]*)$/;
...
// Common username patterns with dots that must never be treated as domains
const EXCLUDED_DOTTED_USERNAMES = new Set([
    "user.name",
    "first.name",
    "last.name",
    "full.name",
...
function isUrlOrDomain(left) {
    ...
    const hostPart = value.split(/[\/?#:]/)[0].toLowerCase();
    if (!hostPart || !hostPart.includes(".")) return false;
    if (IPV4_RE.test(hostPart)) return true;
    if (EXCLUDED_DOTTED_USERNAMES.has(hostPart)) return false;
...
function isUsername(login, password) {
    ...
    // A password in a combo line cannot be a URL scheme, protocol-relative URL, www URL, URL with path, or IP address
    if (
        password &&
        (password.startsWith("//") ||
            SCHEME_URL_RE.test(password) ||
            /^www\./i.test(password) ||
            IPV4_RE.test(password) ||
            (password.includes("/") && isUrlOrDomain(password)))
    ) {
        return false;
    }
    return true;
